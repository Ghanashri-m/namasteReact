...
A: `Parcel/Webpack` is type of a web application bundler used for development and productions purposes or to power our application with different type functionalities and features.
...
A: `Tree shaking` is process of removing the unwanted code that we do not use while developing the application by the bundlers.
...
## Q: What is `browserslist` in package.json?
...
* Let's see each individual query in the above example example:
